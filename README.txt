...
data grid and one pertaining to the chart then added together into the mat-drawer. I would probably add some additional endpoints for flexibility. Then I would start looking at
how best to make the code more extensible. I did not get around to enabling sorting but might be nice. In this case I would probably start with an IRequester interface (via Autofac just my usual DI tool) to make REST api calls to endpoints in the case that we wanted to expand 
